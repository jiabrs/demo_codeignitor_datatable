...
$(document).ready(function(){
	...
	$("a#lookup_role").click(function(event) {
	...
	$('form#search').submit(function() {
		$.ajax({
			...
			success: function(data) {
				...
				$('span#search_result').text('Found '+count.toString()+' role'+s)
			}
		});
	...
	$('form#search_results').submit(function() {
		...
		$('select#result option:selected').each( function () {
			li += '<li class="ui-widget ui-corner-all">';
			...
			li += '<input type="hidden" name="role_id[]" value="'+this.value+'" />';
			li += $(this).text()+'</li>';
		});
		
		$('ul#assigned_roles').append(li);
